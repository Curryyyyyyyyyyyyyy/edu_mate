...
如果已经开始了该测试，调用此接口会返回 400：已开始过该测试，请直接提交。此时应改用 2.4 获取作答进度 获取 attempt_id 并继续作答。
...
answers 为 {question_id: answer} 的映射，仅包含已保存过答案的题目。如果 status 为 submitted，则不能再修改答案。attempt_id 可从 2.2 获取测试详情 的 attempt.id 字段获取。
...
所有题目都在请求体中一次性提交（传统模式）
部分逐题保存 + 部分在提交时传参
全部逐题保存，提交时只需传空数组 {"answers": []}
2.6 提交答案
POST /api/student/courses/{course_id}/quizzes/{quiz_id}/attempts/{attempt_id}/submit
功能说明： 提交所有题目的答案，系统自动批改：客观题立即给分，简答题调用 MiniMax 批改。
